    ...
    return (
      <div className="min-h-screen pt-28 pb-16 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-theme-secondary font-inter">Loading property details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen pt-28 pb-16 flex items-center justify-center text-center">
        <div>
          ...
          <h2 className="font-outfit font-bold text-theme-primary text-2xl mb-2">Property Not Found</h2>
  ...
  return (
    <div className="min-h-screen pt-28 pb-16 page-enter">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Breadcrumb */}
        <div className="flex items-center gap-2 mb-8 text-sm font-inter text-theme-muted">
          <Link href="/" className="hover:text-theme-secondary transition-colors">Home</Link>
          <i className="uil uil-angle-right" />
          <Link href="/listings" className="hover:text-theme-secondary transition-colors">Listings</Link>
          <i className="uil uil-angle-right" />
          <span className="text-theme-secondary">{property.title}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Title row */}
            <div>
              <h1 className="font-outfit font-extrabold text-2xl sm:text-3xl text-theme-primary mb-2">{property.title}</h1>
              <div className="flex items-center gap-2 text-theme-tertiary font-inter text-sm">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={label} className="glass-card p-4 text-center">
                  ...
                  <div className="font-outfit font-bold text-theme-primary text-sm">{val}</div>
                  <div className="text-theme-muted text-xs font-inter mt-0.5">{label}</div>
                </div>
              ))}
            </div>
              ...
              <div className="p-6">
                {tab === 0 && (
                  <div className="space-y-6">
                    <div>
                      <h3 className="font-outfit font-semibold text-theme-primary text-sm mb-3">Description</h3>
                      <p className="text-theme-secondary font-inter text-sm leading-relaxed">{property.description}</p>
                    </div>
                    <div>
                      <h3 className="font-outfit font-semibold text-theme-primary text-sm mb-3">Amenities</h3>
                      <div className="flex flex-wrap gap-2">
                        {property.amenities.map((a) => (
                          <span key={a} className="px-3 py-1.5 rounded-xl text-xs font-inter text-theme-secondary"
                ...
                {tab === 1 && (
                  <div>
                    <h3 className="font-outfit font-semibold text-theme-primary text-sm mb-4">Utilities Included in Rent</h3>
                      ...
                      <div className="space-y-2">
                        {property.utilitiesIncluded.map((u) => (
                          <div key={u} className="flex items-center gap-3 p-3 rounded-xl"
                            ...
                            <span className="text-theme-secondary font-inter text-sm">{u}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-theme-muted font-inter text-sm">No utilities included. Tenant is responsible for all utility connections.</p>
                    )}
                ...
                {tab === 2 && (
                  <div>
                    <div className="flex items-start gap-4 p-5 rounded-xl mb-4"
                      ...
                      <div>
                        <h3 className="font-outfit font-semibold text-theme-primary text-sm mb-1.5">Pet Policy</h3>
                        <p className="text-theme-secondary font-inter text-sm leading-relaxed">{property.petPolicy}</p>
                      </div>
                    </div>
                    <p className="text-theme-muted font-inter text-xs">
          ...
          <div className="space-y-5">
            ...
            <div className="glass-card p-6">
              <div className="mb-4">
                <div className="text-theme-muted text-xs font-inter mb-1">Monthly Rental</div>
                <div className="font-outfit font-extrabold text-2xl text-gold-500">
                  LKR {property.monthlyRate.toLocaleString()}
                </div>
                <div className="text-theme-muted text-xs font-inter mt-1">per month</div>
              </div>

              {/* Cost calculator */}
              <div className="space-y-2 py-4" style={{ borderTop: '1px solid var(--border-light)' }}>
                <div className="flex justify-between text-sm font-inter">
                  <span className="text-theme-secondary">Security Deposit (2 mo.)</span>
                  <span className="text-theme-primary font-outfit font-semibold">LKR {securityDeposit.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm font-inter">
                  <span className="text-theme-secondary">Advance Rent (1 mo.)</span>
                  <span className="text-theme-primary font-outfit font-semibold">LKR {advanceRent.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm font-inter pt-2 mt-2" style={{ borderTop: '1px solid var(--border-light)' }}>
                  <span className="text-theme-primary font-outfit font-bold">Total Move-in Cost</span>
            ...
            <div className="glass-card p-5 space-y-3">
              <h4 className="font-outfit font-semibold text-theme-primary text-sm">Availability Info</h4>
              <div className="flex items-center gap-3 text-sm font-inter">
                <i className="uil uil-calendar-alt text-gold-500 text-lg" />
                <div>
                  <div className="text-theme-muted text-xs mb-0.5">Available From</div>
                  <div className="text-theme-primary">{new Date(property.availableFrom).toLocaleDateString('en-LK', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
                </div>
              </div>
              <div className="flex items-center gap-3 text-sm font-inter">
                <i className="uil uil-map text-gold-500 text-lg" />
                <div>
                  <div className="text-theme-muted text-xs mb-0.5">District</div>
                  <div className="text-theme-primary">{property.district}</div>
                </div>
              </div>
            </div>
            ...
            <div className="glass-card p-5">
              <p className="text-theme-secondary font-inter text-xs mb-3 leading-relaxed">
